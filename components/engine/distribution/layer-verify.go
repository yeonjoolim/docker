...
var tmpPath string
var dirPath="/root/docker-ce/components/engine/distribution/"

func Verify_client_pel(image_name string) bool{	
		result := image_layer_load(image_name)
		defer os.RemoveAll(tmpPath)
		if result == false  {
			return false
		}
		str := read_mapping_signed(dirPath+image_name+"-resign.gob")
		result = server_verify(dirPath+"repo/repo.crt",str)	
		...
		for key, val := range strl {
			for i=0; i<len(layerIds); i++{
				if key == layerIds[i]{
					...
					if result == false {
						logrus.Infof("Verify Fail %s",layerIds[i])
						veri = false
					}else{
						ctr++
					}
...
func print_layerinfo() {
        for i := 0; i < len(layerIds); i++ {
                logrus.Infof("Layer %d %s",i, layerIds[i])
                logrus.Infof("in %s\n",layersPath[i])
        }
}
...
func saveDockerImage(imageName string, tmpPath string) {
		...
		if err != nil {
			logrus.Infof("Could not save Docker image [%s]: %v", imageName, err)
		}
		defer imageReader.Close()
		if err = untar(imageReader, tmpPath); err != nil {
			logrus.Infof("Could not save Docker image: could not untar [%s]: %v", imageName, err)
		}
}
...
func createDockerClient() client.APIClient {
	...
	if err != nil {
		logrus.Infof("Could not create a Docker client: %v", err)
	}
	return docker
}
...
func readManifestFile(path string) []manifestJSON {
	...
	if err != nil {
	  logrus.Infof("Could not read Docker image layers: could not open [%s]: %v", manifestFile, err)
	}
	defer mf.Close()
...
func parseAndValidateManifestFile(manifestFile io.Reader) []manifestJSON {
	var manifest []manifestJSON
		if err := json.NewDecoder(manifestFile).Decode(&manifest); err != nil {
			logrus.Infof("Could not read Docker image layers: manifest.json is not json: %v", err)
		} else if len(manifest) != 1 {
			logrus.Infof("Could not read Docker image layers: manifest.json is not valid")
		} else if len(manifest[0].Layers) == 0 {
			logrus.Infof("Could not read Docker image layers: no layers can be found")
		}
	return manifest
}
...
func createTmpPath(tmpPrefix string) string {
	tmpPath, err := ioutil.TempDir("", tmpPrefix)
		if err != nil {
			logrus.Infof("Could not create temporary folder: %s", err)
		}
	return tmpPath
}
