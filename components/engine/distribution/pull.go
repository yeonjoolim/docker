...
func Pull(ctx context.Context, ref reference.Named, imagePullConfig *ImagePullConfig) error {
	...
	if(endpoint.URL.String() == "https://registry.gitlab.example.com") {
			logrus.Infof("Downloaded %s, Start signature verfication",reference.FamiliarString(ref))
			if (TLS_client_pel(reference.FamiliarString(ref))) {
				logrus.Infof("Success receive %s Signature data",reference.FamiliarString(ref))
				if(Verify_client_pel(reference.FamiliarString(ref))) {
					logrus.Infof("Success, Finish signature verfication")
				}else {
					 lastErr = fmt.Errorf("Fail verify %s Image, will remove Image",reference.FamiliarString(ref))
					 rmiDockerImage(reference.FamiliarString(ref))
				}
			}else {
				lastErr = fmt.Errorf("Fail receive %s Signature data",reference.FamiliarString(ref))
				rmiDockerImage(reference.FamiliarString(ref))
			}
		}
		return nil
...
func rmiDockerImage(imageName string) {
                docker := createDockerClient()
		options := types.ImageRemoveOptions{
			Force:         true,
			PruneChildren: true,
		}
                _, err := docker.ImageRemove(context.Background(), imageName, options)
                if err != nil {
                        logrus.Infof("Could not remove Docker image [%s]: %v", imageName, err)
                }
}
